...
class Verify {
    ...
    public function index_verify($id = '') {
        // 图片宽(px)
        $this->imageW || $this->imageW = $this->length*$this->fontSize*1.5 + $this->length*$this->fontSize/2;
        // 图片高(px)
        $this->imageH || $this->imageH = $this->fontSize * 2.5;
...
//                die();
                $code[$i] = $this->codeSet[mt_rand(0, strlen($this->codeSet)-1)];
                imagettftext($this->_image, $this->fontSize, 0, $codeNX, $this->fontSize*1.7, imagecolorallocate($this->_image,255,255,255), $this->fontttf, $code[$i]);
